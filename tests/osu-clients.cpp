...
void check_client_initialization(ClientBase& client, int expected_beatmap_count) {
    ...
    const auto beatmaps = client.search_beatmaps(make_search_options());
...
void check_client_search(ClientBase& client) {
    ...
    const auto filtered = client.search_beatmaps(make_search_options("artist=\"glass beach\" ar>=8"));
    REQUIRE_FALSE(filtered.empty());

    const auto sorted_by_duration = client.search_beatmaps(make_search_options("", "duration"));
    ...
    auto duration_options = make_search_options();
    duration_options.has_duration = true;

    const auto with_duration = client.search_beatmaps(duration_options);
    REQUIRE(with_duration.size() <= all_beatmaps.size());
...
TEST_CASE("clients initialization", "[clients]") {
    SECTION("stable") {
        ...
        check_client_initialization(*client, STABLE_BEATMAP_COUNT);
        check_temp_collection(*client, true, "temp-stable");
    }

    SECTION("lazer") {
        auto client = make_client("lazer");
        check_client_initialization(*client, LAZER_BEATMAP_COUNT);
        check_temp_collection(*client, false, "temp-lazer");
    }
}

TEST_CASE("clients search", "[clients]") {
    SECTION("stable") {
        auto client = make_client("stable");
        check_client_search(*client);
    }
