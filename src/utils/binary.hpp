...
inline T byteswap(T value) {
    ...
    if constexpr (sizeof(U) == 1) {
        ...
        uvalue = static_cast<U>(((uvalue & 0x00000000000000FFull) << 56) | ((uvalue & 0x000000000000FF00ull) << 40) |
...
inline std::string normalize_and_lower(std::string_view s) {
    ...
    utf8proc_ssize_t len =
        utf8proc_map(reinterpret_cast<const utf8proc_uint8_t*>(s.data()), static_cast<utf8proc_ssize_t>(s.size()),
                     &result, static_cast<utf8proc_option_t>(UTF8PROC_DECOMPOSE | UTF8PROC_CASEFOLD | UTF8PROC_NLF2LS));
