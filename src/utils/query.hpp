...
namespace query {
inline std::pair<QueryOp, size_t> parse_operator(std::string_view sv,
                                                 size_t pos) {
  char c1 = sv[pos];
  char c2 = sv[pos + 1];
...
inline ParsedQuery parse(std::string_view data) {
  ...
  for (size_t i = 0; i < data.length(); i++) {
    ...
    case ParseState::KEY: {
      ...
      // is a operator?
      if (m_state.key_start < i &&
          OP_START_TABLE[static_cast<unsigned char>(c)]) {
        auto [op, size] = parse_operator(data, i);

        // invalidate duplicated op's
        if (!is_last && c != '=' && data[i + 1] == c) {
          result.content +=
              data.substr(m_state.key_start, i - m_state.key_start);
          m_state.reset();
        } else {
          m_state.op = op;
    ...
    case ParseState::VALUE: {
      ...
      result.tokens.push_back({
          .key = std::string(data.substr(m_state.key_start,
                                         m_state.op_start - m_state.key_start)),
