...
namespace binary {
inline bool gzip_decompress(const std::vector<uint8_t> &input,
  ...
  const size_t max_decompressed_size =
      256 * 1024 * 1024; // 256 MiB safety cap for inflate.
  ...
  while (member_offset < size) {
    ...
    if (data[member_offset] != 0x1F || data[member_offset + 1] != 0x8B ||
        data[member_offset + 2] != 0x08) {
      return false;
    }
    ...
    if (flags & 0x02) {
      ...
      const mz_ulong header_crc =
          mz_crc32(MZ_CRC32_INIT, data + member_offset, offset - member_offset);
    ...
    const size_t member_output_offset =
        temp_output.size() - static_cast<size_t>(stream.total_out);
    const mz_ulong actual_crc =
        mz_crc32(MZ_CRC32_INIT, temp_output.data() + member_output_offset,
...
inline bool gzip_compress(const std::vector<uint8_t> &input,
                          std::vector<uint8_t> &output) {
  mz_stream stream{};
