    ...
    inline bool gzip_decompress(const std::vector<uint8_t>& input, std::vector<uint8_t>& output) {
        ...
        while (member_offset < size) {
            ...
            if (flags & 0x04) {
                ...
                const uint16_t xlen = static_cast<uint16_t>(data[offset]) | (static_cast<uint16_t>(data[offset + 1]) << 8);
                offset += 2;
                if (offset + xlen > size) {
                    return false;
                }
            ...
            const mz_ulong expected_crc =
                static_cast<mz_ulong>(data[footer_offset]) | (static_cast<mz_ulong>(data[footer_offset + 1]) << 8) |
                (static_cast<mz_ulong>(data[footer_offset + 2]) << 16) | (static_cast<mz_ulong>(data[footer_offset + 3]) << 24);

            // ISIZE: uncompressed size modulo 2^32.
            const mz_ulong expected_size =
                static_cast<mz_ulong>(data[footer_offset + 4]) | (static_cast<mz_ulong>(data[footer_offset + 5]) << 8) |
                (static_cast<mz_ulong>(data[footer_offset + 6]) << 16) | (static_cast<mz_ulong>(data[footer_offset + 7]) << 24);

            const size_t member_output_offset = temp_output.size() - static_cast<size_t>(stream.total_out);
            const mz_ulong actual_crc =
                mz_crc32(MZ_CRC32_INIT, temp_output.data() + member_output_offset, static_cast<size_t>(stream.total_out));
