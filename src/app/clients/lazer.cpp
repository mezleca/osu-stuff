...
LazerClient::LazerClient(ClientOptions options) : m_options(std::move(options)) {
    ...
    m_realm = std::make_unique<realm::db>(
        realm::open<
            realm::BeatmapDifficulty, realm::BeatmapUserSettings, realm::RealmUser, realm::Ruleset, realm::File,
            realm::RealmNamedFileUsage, realm::BeatmapMetadata, realm::BeatmapCollection, realm::BeatmapSet, realm::Beatmap>(
            config
        )
    );
...
std::vector<std::string> LazerClient::fetch_missing_beatmaps_from_collections(std::string_view collection_name) {
