...
const render_items = (id, extra = { force: false, check: false }) => {
    ...
    const base_index = Math.max(
        0,
        Math.floor(scroll_top / item_total_height) - BUFFER_SIZE,
    );
    const items_in_view =
        Math.ceil(visible_height / item_total_height) + BUFFER_SIZE * 2;
    const max_amount = Math.min(
        base_index + items_in_view,
        virtual_list.length,
    );
    ...
    for (let i = base_index; i < max_amount; i++) {
        ...
        if (!content?.element) {
            const new_item_content = virtual_list.create(i);
            content = {
                element: new_item_content.element(),
                id: new_item_content.id,
            };
...
export const create_virtual_list = (
    options = { id: 0, target: null, create: (i) => {}, get: () => {} },
) => {
    const virtual_height = create_element(
        `<div class="virtual-height" id="${options.id}"></div>`,
    );
    const list_target = create_element(
        `<div class="virtual-list" id="${options.id}"></div>`,
    );
    const virtual_container = create_element(
    ...
    const virtual_list = {
        ...
        initialize: () => {
            ...
            const base_size = get_element_size(
                options.create(0).element(),
                options.id,
            );
            ...
            list_target.addEventListener(
                "scroll",
                debounce(() => render(options.id), 30),
            );
        ...
        cleanup: () => {
            ...
            virtual_list.last_rendered;
            list_target.removeEventListener(
                "scroll",
                debounce(() => render(options.id, {}), 20),
            );
