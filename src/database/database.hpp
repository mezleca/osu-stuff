...
public:
    ...
    DatabaseBinding(Loader loader, Saver saver) : m_loader(std::move(loader)), m_saver(std::move(saver)), m_value(m_loader()) {}
...
#define DATABASE_FIELD(database, field)                                                                                          \
    (database)->bind(                                                                                                            \
        ...
        [](auto& change, auto& config, const auto& value) { change.set((config.field), value); }                                 \
    )
