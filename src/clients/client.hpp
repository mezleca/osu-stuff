...
// stable -> result
    explicit OsuBeatmap(const LegacyBeatmap& b)
        : artist(b.artist), title(b.title), creator(b.creator), difficulty(b.difficulty), audio_file_name(b.audio_file_name),
          md5(b.md5), source(b.source), osu_file_name(b.osu_file_name), tags(b.tags), artist_unicode(b.artist_unicode),
          title_unicode(b.title_unicode), duration(b.duration), approach_rate(b.approach_rate), circle_size(b.circle_size),
          overall_difficulty(b.overall_difficulty), hp_drain(b.hp_drain), slider_velocity(b.slider_velocity),
          last_modification_time(b.last_modification_time), hitcircle(b.hitcircle), sliders(b.sliders), spinners(b.spinners),
          drain_time(b.drain_time), total_time(b.total_time), audio_preview_time(b.audio_preview_time),
          difficulty_id(b.difficulty_id), beatmap_id(b.beatmap_id), mode((BeatmapGamemode)b.mode),
          status((BeatmapStatus)b.status) {}

// lazer -> result
    explicit OsuBeatmap(const realm::managed<realm::Beatmap>& b)
        : artist(b.Metadata ? detach_or_empty(b.Metadata->Artist) : ""),
          title(b.Metadata ? detach_or_empty(b.Metadata->Title) : ""),
          creator(b.Metadata && b.Metadata->Author ? detach_or_empty(b.Metadata->Author->Username) : ""),
          difficulty(detach_or_empty(b.DifficultyName)),
          audio_file_name(b.Metadata ? detach_or_empty(b.Metadata->AudioFile) : ""), md5(detach_or_empty(b.MD5Hash)),
          source(b.Metadata ? detach_or_empty(b.Metadata->Source) : ""),
          tags(b.Metadata ? detach_or_empty(b.Metadata->Tags) : ""),
          artist_unicode(b.Metadata ? detach_or_empty(b.Metadata->ArtistUnicode) : ""),
          title_unicode(b.Metadata ? detach_or_empty(b.Metadata->TitleUnicode) : ""), duration(b.Length.detach()),
          ...
          last_modification_time(detach_time_ms(b.LastLocalUpdate)), hitcircle(0), sliders((int)b.EndTimeObjectCount.detach()),
          spinners(0), drain_time((int)b.Length.detach()), total_time((int)b.Length.detach()),
          audio_preview_time(b.Metadata ? (int)b.Metadata->PreviewTime.detach() : 0), difficulty_id((int)b.OnlineID.detach()),
          beatmap_id(b.BeatmapSet ? (int)b.BeatmapSet->OnlineID.detach() : 0), mode(detach_mode(b.Ruleset)),
...
public:
    ...
    [[nodiscard]] virtual OsuBeatmap* get_beatmap(const std::string& md5);
