    ...
    // stable -> result
        explicit OsuBeatmap(const LegacyBeatmap& b)
            : artist(b.artist), title(b.title), creator(b.creator), difficulty(b.difficulty), audio_file_name(b.audio_file_name),
              ...
              artist_unicode(b.artist_unicode), title_unicode(b.title_unicode), duration(b.duration),
              approach_rate(b.approach_rate), circle_size(b.circle_size), overall_difficulty(b.overall_difficulty),
              hp_drain(b.hp_drain), slider_velocity(b.slider_velocity), last_modification_time(b.last_modification_time),
              hitcircle(b.hitcircle), sliders(b.sliders), spinners(b.spinners), drain_time(b.drain_time),
              total_time(b.total_time), audio_preview_time(b.audio_preview_time), difficulty_id(b.difficulty_id),
              beatmap_id(b.beatmap_id), mode((BeatmapGamemode)b.mode), status((BeatmapStatus)b.status) {}
    ...
    // lazer -> result
        explicit OsuBeatmap(const realm::managed<realm::Beatmap>& b)
            : artist(b.Metadata ? detach_or_empty(b.Metadata->Artist) : ""),
              ...
              source(b.Metadata ? detach_or_empty(b.Metadata->Source) : ""), osu_file_name(""),
              tags(b.Metadata ? detach_or_empty(b.Metadata->Tags) : ""), searchable(""),
              ...
              last_modification_time(detach_time_ms(b.LastLocalUpdate)), hitcircle(0),
              ...
              mode(detach_mode(b.Ruleset)), status((BeatmapStatus)b.Status.detach()) {}
        ...
        void build_search() {
            searchable = binary::normalize_and_lower(
                std::format(
                    "{} {} {} {} {} {} {} {} {} {}", title, title_unicode, artist, artist_unicode, creator, difficulty, source,
                    tags, difficulty_id, beatmap_id
                )
            );
        }
    };
