...
UINotificationManager::UINotificationManager(UI& ui)
    : LayerContainer("notifications"), m_more_notifications(ui, LogNotificationLevel::PLACEHOLDER, "") {
    set_visible(true);
...
bool UINotificationManager::add(std::unique_ptr<UINotification> notification) {
    ...
    UINotification* added = notification.get();
    if (!attach(std::move(notification))) {
        return false;
    }
...
bool UINotificationManager::remove(UINotification* to_remove) {
    ...
    return Node::remove(*to_remove) != nullptr;
}
...
void UINotificationManager::set_position(NotificationPosition position) {
...
void UINotificationManager::clear_notifications() {
    while (!children().empty()) {
        Node::remove(*children().back());
    }
}
...
void UINotificationManager::draw_children() {
    ...
    const ImVec2 initial_offset = {m_position == NotificationPosition::Left ? 5.0F : -5.0F, m_header_height + 10.0F};
    const float more_height = std::max(48.0F, m_more_notifications.layout().visual_rect().size().y);
    ...
    for (auto it = children().rbegin(); it != children().rend(); ++it) {
        ...
        if (!notification->visually_visible()) {
            m_pending_removals.push_back(notification);
            continue;
        }
        ...
        const ImVec2 size = notification->layout().visual_rect().size();
        offset.y += size.y + SPACING;
        ++index;
    }

    for (UINotification* notification : m_pending_removals) {
        Node::remove(*notification);
    }
