...
void custom_imgui::end_child(ChildState& state, customChildBorder flags, ImU32 color, float thickness) {
    ...
    if (flags & BORDER_ALL) {
        ...
        if (flags & BORDER_TOP) {
            dl->AddLine({min.x, min.y}, {max.x, min.y}, color, thickness);
        }
        if (flags & BORDER_BOTTOM) {
            dl->AddLine({min.x, max.y}, {max.x, max.y}, color, thickness);
        }
        if (flags & BORDER_LEFT) {
            dl->AddLine({min.x, min.y}, {min.x, max.y}, color, thickness);
        }
        if (flags & BORDER_RIGHT) {
            dl->AddLine({max.x, min.y}, {max.x, max.y}, color, thickness);
        }
    };
}
