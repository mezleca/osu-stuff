  ...
  try {
    ...
    if (version >= 7) {
      std::vector<uint8_t> compressed(
          buffer.begin() + static_cast<std::ptrdiff_t>(cursor.offset),
          buffer.end());
...
bool osdb_parser::write() {
  ...
  const std::string version_string =
      data->version_string.empty() ? "o!dm8min" : data->version_string;
  const int version = osdb_version_to_code(version_string);
  ...
  for (const auto &collection : data->collections) {
    ...
    if (version >= 3) {
      binary::write_i32(content,
                        static_cast<int>(collection.hash_only_beatmaps.size()));
