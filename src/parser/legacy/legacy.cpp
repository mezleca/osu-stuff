...
static LegacyFloatPair read_int_float_pair(binary::BinaryCursor &cursor,
                                           bool use_float) {
...
static std::vector<LegacyFloatPair>
read_star_ratings(binary::BinaryCursor &cursor, bool use_float) {
...
static void write_int_float_pair(std::vector<uint8_t> &buffer,
                                 const LegacyFloatPair &pair, bool use_float) {
...
static void write_star_ratings(std::vector<uint8_t> &buffer,
                               const std::vector<LegacyFloatPair> &ratings,
                               bool use_float) {
...
bool legacy_parser::parse(std::filesystem::path &location,
                          OsuLegacyDatabase *data) {
  ...
  try {
    ...
    data->beatmaps.clear();
    data->beatmaps.reserve(
        static_cast<size_t>(std::max(0, data->beatmaps_count)));
...
bool legacy_parser::write(std::filesystem::path &location,
                          OsuLegacyDatabase *data) {
  ...
  for (const auto &beatmap : data->beatmaps) {
    ...
    if (old_diff_format) {
      binary::write_i16(entry,
                        static_cast<int16_t>(beatmap.unknown.value_or(0)));
    }
