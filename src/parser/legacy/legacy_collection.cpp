...
bool legacy_collection_parser::parse(const std::string location,
  ...
  try {
    ...
    data->collections.reserve(
        static_cast<size_t>(std::max(0, data->collections_count)));
    ...
    for (int i = 0; i < data->collections_count; i++) {
      ...
      collection.beatmap_md5.reserve(
          static_cast<size_t>(std::max(0, collection.beatmaps_count)));
...
bool legacy_collection_parser::write(const std::string location,
                                     OsuLegacyCollection *data) {
